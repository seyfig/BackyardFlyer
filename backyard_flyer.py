...
class BackyardFlyer(Drone):
    ...
    def local_position_callback(self):
        ...
        if (self.flight_state == States.TAKEOFF or
            ...
            if altitude > 0.95 * self.target_position[2]:
                d_x = self.local_position[0] - self.target_position[0]
                d_y = self.local_position[1] - self.target_position[1]
                dist = np.sqrt(d_x * d_x + d_y * d_y)
                if dist < 0.3:
                    # If all waypoints are visited
                    if self.at_waypoint == 4:
    ...
    def waypoint_transition(self):
        ...
        wp = self.all_waypoints[self.at_waypoint]
        for i in range(3):
            self.target_position[i] = wp[i]
        print("target_position:", self.target_position)
        self.at_waypoint += 1
        self.flight_state = States.WAYPOINT
        self.cmd_position(*wp)
